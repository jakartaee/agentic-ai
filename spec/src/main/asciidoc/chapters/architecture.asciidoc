...
This chapter provides an architectural overview of Jakarta Agentic AI, describing 
how agents are structured, how they integrate with Jakarta EE, and the key design 
patterns.

=== Agent Architecture

An agent in Jakarta Agentic AI is a CDI bean that orchestrates a flexible workflow. 
Workflows always begin with a trigger and may include any combination of the following phases:

1. **Trigger Phase** (required): Detects an event or condition that initiates the workflow
2. **Decision Phases** (optional): Analyze workflow state and determine whether to proceed
...
**Phase Flexibility**: Decisions and actions can be intermixed in any sequence, allowing 
agents to implement simple linear workflows or complex conditional branching patterns.
