...
- CycloneDX SBOMs are generated only for the `api` module (`cyclonedx.skip` is `true` in the root pom, `false` in `api`). The aggregator's BOM would otherwise describe unpublished modules.
